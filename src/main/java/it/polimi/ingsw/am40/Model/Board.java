...
//                System.out.println(tile);
    ...
    private boolean isFree(Position pos) {
        if (grid.containsKey(pos.getKey())) {
            return grid.get(pos.getKey()).getColor() == TileColor.NOCOLOR || grid.get(pos.getKey()).getType() == TileType.EMPTY;
        }
        return true;
    }
            ...
            if(pl.getSelectedPositions().size()<MAX_TILE){
                for (Position position : startingPickable) {
                    if (!pickableTiles.contains(position)) {
                        ...
                        if (!isInLine(pos, position, pl.getSelectedPositions())) {
                            pickableTiles.remove(position);
                        }
                    }
                }
            }
